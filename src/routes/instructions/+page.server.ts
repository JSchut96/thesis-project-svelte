...
export async function load({locals}){
    ...
    if(layoutIndex == 0) {
        try {
            ...
            console.log("Failed to update Started field for paticipant", locals.participant.sessionId, new Date().toISOString());
...
export const actions: Actions = {
    default: async ({ request, locals }) => {
        ...
        switch (layoutOrder[layoutIndex]){
            ...
            default:
                console.log("Failed to redirect to the task. ", participant.sessionId, new Date().toISOString());
