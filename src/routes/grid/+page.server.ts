...
export async function load({ locals }) {
  ...
  let existingProfile
  try {
    existingProfile = await prisma.gridMeasurements.findUnique({
  ...
  if (!existingProfile) {
    try {
      ...
      console.log(
        'Created gridMeasurements entry for participant ',
  ...
  console.log(
    'Start fetching grid movies for',
    locals.participant.sessionId,
    new Date().toISOString()
  )
