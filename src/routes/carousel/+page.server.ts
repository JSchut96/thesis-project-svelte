...
export async function load({ locals }) {
  ...
  const layout = locals.participant.moviePages?.carousel
  ...
  let existingProfile
  try {
    existingProfile = await prisma.carouselMeasurements.findUnique({
  ...
  if (!existingProfile) {
    try {
      await prisma.carouselMeasurements.create({
        data: {
      ...
      console.log(
        'Created carouselMeasurements entry for participant ',
    ...
    if (existingProfile.endTime === null) {
      ...
      console.error(
        'Measurement session already completed for ',
        locals.participant.sessionId,
        new Date().toISOString()
      )
  ...
  console.log(
    'Start fetching carousel movies for',
    locals.participant.sessionId,
    new Date().toISOString()
  )
  ...
  try {
    for (const genre of genreOrder) {
      ...
      const movieIds = entries.map(
        (entry: { movieId: number; rating: number }) => entry.movieId
      )
      movieDetailsByGenre[genre] = await fetchMoviesById(movieIds)
    }
    console.log(
      'Done fetching carousel movies for',
      locals.participant.sessionId,
      new Date().toISOString()
    )
