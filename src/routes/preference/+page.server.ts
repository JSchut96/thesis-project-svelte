...
export async function load({locals}) {
  console.log('Start fetching preference movies for', locals.participant?.sessionId, new Date().toISOString())

  try {
    const movies = await fetchMoviesById(elicitation_movies)
    console.log('Done fetching preference movies for!', locals.participant?.sessionId, new Date().toISOString())
    return { movies }
  } catch (err) {
